...
export function NeedsAttention() {
  ...
  const problemRows = computed.filter(r => {
    const marginPct = r.receivablePkr !== 0 ? (r.netMarginPkr / r.receivablePkr) * 100 : 0;
    return r.netMarginPkr < 0 || marginPct < 10;
  });
        ...
        <table className="w-full">
          ...
          <tbody>
            {recordsLoading ? (
              ...
              <tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-muted-foreground">Nothing needs attention</td></tr>
            ) : (
              problemRows.map(r => {
                const marginPct = r.receivablePkr !== 0 ? (r.netMarginPkr / r.receivablePkr) * 100 : 0;
