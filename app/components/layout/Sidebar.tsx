...
export default function Sidebar({ open, onToggle }: SidebarProps) {
  ...
  const [financialsOpen, setFinancialsOpen] = useState(
    financialsItems.some(item => pathname.startsWith(item.href))
  );
  ...
  const hasAnyFinancials = financialsItems.some(item => canAccess(item.permission));
  const isFinancialsActive = financialsItems.some(item => pathname.startsWith(item.href));
