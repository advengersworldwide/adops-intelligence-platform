...
export function WaterfallChart({ stages, currency }: { stages: WaterfallStage[]; currency?: string }) {
  ...
  return (
    ...
    <div className="mt-2 flex flex-wrap justify-center gap-3 text-[10px] text-muted-foreground">
      <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-sm" style={{ backgroundColor: "hsl(221,83%,53%)" }} /> Running total</span>
      <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-sm" style={{ backgroundColor: "hsl(0,84%,60%)" }} /> Decrease</span>
      <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-sm" style={{ backgroundColor: "hsl(160,84%,39%)" }} /> Total</span>
    </div>
    </>
  );
}
