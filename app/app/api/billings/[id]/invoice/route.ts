...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  if (b.invoiceGeneratedAt) return NextResponse.json(await mapBilling(b)); // already invoiced — idempotent

  const now = new Date();
  // Client bills carry a CBILL code from creation; only legacy bills without one need one generated here.
  let invoiceCode = b.invoiceCode;
  if (!invoiceCode) {
    ...
    invoiceCode = "CBILL-" + formatPoCode(prefix, now, Number(value) + 1);
  }
