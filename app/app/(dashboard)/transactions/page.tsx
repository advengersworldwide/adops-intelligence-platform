...
export default function BillingPage() {
  ...
  const computed = (records ?? []).map(r => ({
    ...
    ...computeRow({
      ...
      salesTaxPct: String(r.salesTaxPct),
      remittanceTaxPct: String(r.remittanceTaxPct),
      withholdingTaxPct: String(r.withholdingTaxPct),
...
function AddRecordDialog({ open, onClose, platforms, buyingHouses, clients, onSuccess, defaultValues, recordId }: {
  ...
  useEffect(() => {
    if (selectedBH) {
      ...
      form.setValue("bulkDiscountPct", selectedBH.bulkDiscountPct != null ? Number(selectedBH.bulkDiscountPct) : 0);
    }
