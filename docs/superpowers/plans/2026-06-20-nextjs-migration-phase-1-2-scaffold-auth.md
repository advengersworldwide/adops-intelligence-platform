...
`next-env.d.ts` is generated by the build but is git-ignored (per Task 1's `.gitignore`, matching Next.js convention), so it is NOT committed.

```bash
git add artifacts/web/postcss.config.mjs artifacts/web/app/globals.css artifacts/web/app/layout.tsx artifacts/web/app/page.tsx
