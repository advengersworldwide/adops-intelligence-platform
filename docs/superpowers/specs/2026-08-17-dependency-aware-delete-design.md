...
- **Fully closing the read/write race on cascades.** The delete endpoint re-resolves the impact *inside* the transaction, so the resolve and the deletes share one connection and the gap between them is small. It is not zero. Postgres defaults to READ COMMITTED, where each **statement** takes its own snapshot rather than each transaction, so a row committed between the resolver's last query and the `DELETE` is still visible to that `DELETE`. Blockers are protected regardless — `RESTRICT` raises `23503`, which the endpoint maps to a 409 — but a **cascade** child inserted inside that window is destroyed without ever appearing in the reviewed tree. A true shared snapshot needs `REPEATABLE READ` plus `40001` serialization-failure handling, which introduces a new failure mode on an irreversible financial operation; `SELECT … FOR UPDATE` on the target is cheaper but only blocks direct children, since grandchildren reference an intermediate blocker rather than the target. Both were considered and deliberately deferred. **So the fingerprint guarantee is "you reviewed the tree as it stood moments before deletion", not "you reviewed exactly what was deleted."** Do not cite it as the stronger claim.
