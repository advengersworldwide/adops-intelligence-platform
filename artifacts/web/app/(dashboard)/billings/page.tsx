...
function generateInvoicePdf(bill: BillDetail) {
  ...
  const rows = bill.transactions.map(t => {
    const c = computeRow({
      ...
      payoutRate: String(t.payoutRate), marginPct: String(t.marginPct),
      forexSellingRate: String(t.forexSellingRate ?? 0), forexBuyingRate: String(t.forexBuyingRate ?? 0),
      salesTaxPct: String(t.salesTaxPct), remittanceTaxPct: String(t.remittanceTaxPct),
      withholdingTaxPct: String(t.withholdingTaxPct), bulkDiscountPct: String(t.bulkDiscountPct ?? 0),
      platformBulkDiscountPct: String(t.platformBulkDiscountPct ?? 0),
    });
    return [
      t.period,
            ...
            <div>
              ...
              <div className="border border-border rounded-lg max-h-64 overflow-y-auto divide-y divide-border">
                {(transactions ?? []).map(t => {
                  const c = computeRow({
                    appsflyerPins: t.appsflyerPins, fraudPins: t.fraudPins,
                    payoutRate: String(t.payoutRate ?? 0), marginPct: String(t.marginPct ?? 0),
                    forexSellingRate: String(t.forexSellingRate ?? 0), forexBuyingRate: String(t.forexBuyingRate ?? 0),
                    salesTaxPct: String(t.salesTaxPct ?? 0), remittanceTaxPct: String(t.remittanceTaxPct ?? 0),
                    withholdingTaxPct: String(t.withholdingTaxPct ?? 0), bulkDiscountPct: String(t.bulkDiscountPct ?? 0),
                    platformBulkDiscountPct: String(t.platformBulkDiscountPct ?? 0),
                  });
