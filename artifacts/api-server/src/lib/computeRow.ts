// Mirrors artifacts/adops/src/lib/computeRow.ts — keep both in sync when formula changes.
...
export function computeRow(r: ComputeRowInput): ComputeRowResult {
  ...
  // Receivable side (invoice to buying house, uses forex selling rate)
  ...
  // Payable side (payment to platform, uses forex buying rate)
